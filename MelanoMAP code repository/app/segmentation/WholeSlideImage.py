...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=10, sthresh_up=255, mthresh=7, close=0, use_otsu=False, 
                    filter_params={'a_t':100}, ref_patch_size=1280, exclude_ids=[], keep_ids=[]):
        """
       ...
       # import pdb;pdb.set_trace()
        ...
        # Thresholding
        if use_otsu:
            img_eq = cv2.equalizeHist(img_med)
            _, img_otsu = cv2.threshold(img_eq, 0, sthresh_up, cv2.THRESH_OTSU + cv2.THRESH_BINARY)
        ...
        if close > 0:
            ...
            # Additional step to remove very black regions
        img_gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        _, dark_regions = cv2.threshold(img_gray, 70, 255, cv2.THRESH_BINARY_INV)  # More aggressive threshold value

     
        # Apply dilation to ensure all black regions are captured
        dilation_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7))  # Adjust kernel size as needed
        dark_regions = cv2.dilate(dark_regions, dilation_kernel, iterations=3)
        # Invert the mask back to combine with img_otsu
        dark_regions = cv2.bitwise_not(dark_regions)

        # Combine img_otsu and dark_regions to exclude the black parts from the otsu mask
        combined_mask = cv2.bitwise_and(img_otsu, dark_regions)
        ...
        contours, hierarchy = cv2.findContours(combined_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
        hierarchy = np.squeeze(hierarchy, axis=(0,))[:, 2:]
        if filter_params: 
            foreground_contours, hole_contours = _filter_contours(contours, hierarchy, filter_params)

        # Dilation process
        # Changed from 70 to 50 of kernel_size
        kernel_size = 70  
        iterations = 2
        ...
        for contour in foreground_contours:
            ...
            cv2.drawContours(mask, [contour], -1, 1, thickness=cv2.FILLED) # type: ignore
            dilated_mask = cv2.dilate(mask, kernel, iterations=iterations)
            dilated_contour, _ = cv2.findContours(dilated_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            dilated_contours.extend(dilated_contour)
        
        # Use dilated contours instead of the original ones
        foreground_contours = dilated_contours

        # Scale contours and holes
        self.contours_tissue = self.scaleContourDim(foreground_contours, scale)
        self.holes_tissue = self.scaleHolesDim(hole_contours, scale)

        # Handle excluded and kept IDs
        if len(keep_ids) > 0:
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        elapsed = time.time()
        n_contours = len(self.contours_tissue) if self.contours_tissue is not None else 0
        print("Total number of contours to process: ", n_contours)
        fp_chunk_size = math.ceil(n_contours * 0.05)
        init = True
        for idx, cont in enumerate(self.contours_tissue if self.contours_tissue is not None else []):
            print('Processing contour', idx, '/', n_contours)
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0,0)):
        ...
        for idx in range(len(contours_tissue)):
            cv2.drawContours(image=tissue_mask, contours=contours_tissue, contourIdx=idx, color=(1), offset=offset, thickness=-1) # type: ignore

            if use_holes:
                cv2.drawContours(image=tissue_mask, contours=contours_holes[idx], contourIdx=-1, color=(0), offset=offset, thickness=-1) # type: ignore
