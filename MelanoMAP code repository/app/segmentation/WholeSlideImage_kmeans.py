...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up=255, mthresh=7, close=0, use_otsu=False, 
        ...
        # Pen mark removal preprocessing step
        img_lab_small = cv2.cvtColor(img_small, cv2.COLOR_RGB2LAB)
        pixel_values = img_lab_small.reshape((-1, 3)).astype(np.float32)
        
        # Define the criteria and apply k-means clustering
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 0.2)
        k = 2  # Number of clusters

        _, labels, centers = cv2.kmeans(pixel_values, k, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS) # type: ignore
        
        centers = np.uint8(centers)
        labels = labels.flatten()
        segmented_image = centers[labels.flatten()] # type: ignore
        segmented_image = segmented_image.reshape(img_lab_small.shape)
        
        # Create a mask for the pen marks (assume the darkest cluster corresponds to pen marks)
        mask_small = labels.reshape(img_lab_small.shape[:2])
        mask_small = (mask_small == np.argmin(np.sum(centers, axis=1))).astype(np.uint8)
        ...
        # Apply the mask to the original image to remove pen marks
        img_processed = img.copy()
        img_processed[mask == 1, :] = [255, 255, 255]  # Replace pen marks with white

        img_hsv = cv2.cvtColor(img_processed, cv2.COLOR_RGB2HSV)  # Convert to HSV space
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0,0)):
        ...
        for idx in range(len(contours_tissue)):
            cv2.drawContours(image=tissue_mask, contours=contours_tissue, contourIdx=idx, color=(1, 1, 1), offset=offset, thickness=-1) # type: ignore

            if use_holes:
                cv2.drawContours(image=tissue_mask, contours=contours_holes[idx], contourIdx=-1, color=(0, 0, 0), offset=offset, thickness=-1) # type: ignore
