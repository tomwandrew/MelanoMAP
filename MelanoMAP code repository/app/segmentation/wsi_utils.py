...
def savePatchIter_bag_hdf5(patch):
    ...
    dset = file['imgs']
    dset.resize(len(dset) + img_shape[0], axis=0) # type: ignore
    dset[-img_shape[0]:] = img_patch # type: ignore

    if 'coords' in file:
        coord_dset = file['coords']
        coord_dset.resize(len(coord_dset) + img_shape[0], axis=0) # type: ignore
        coord_dset[-img_shape[0]:] = (x,y) # type: ignore
...
def DrawMap(canvas, patch_dset, coords, patch_size, indices=None, verbose=0, draw_grid=True):
    if indices is None:
...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level,
                       indices=None, verbose=0, draw_grid=True, patch_save_dir=None, patch_level=0):
    ...
    for idx in range(total):
        ...
        # Saving the patches as images
        # save_path = os.path.join(slide_patch_path, f'{wsi_object.name} [d={vis_level},x={coord[0]},y={coord[1]},w={og_patch_size[0]},h={og_patch_size[1]}].jpg')
        # og_patch = np.array(wsi_object.wsi.read_region(tuple(og_coord), patch_level, og_patch_size).convert("RGB"))
        # og_patch_im = Image.fromarray(og_patch)
        # og_patch_im.save(save_path)
...
def StitchPatches(hdf5_file_path, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1):
    ...
    dset = file['imgs']
    coords = file['coords'][:] # type: ignore
    if 'downsampled_level_dim' in dset.attrs.keys():
        w, h = dset.attrs['downsampled_level_dim'] # type: ignore
    else:
        w, h = dset.attrs['level_dim'] # type: ignore
    print('original size: {} x {}'.format(w, h))
    w = w // downscale
    h = h //downscale
    coords = (coords / downscale).astype(np.int32) # type: ignore
    print('downscaled size for stiching: {} x {}'.format(w, h)) # type: ignore
    print('number of patches: {}'.format(len(dset))) # type: ignore
    img_shape = dset[0].shape # type: ignore
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False,
                  bg_color=(0,0,0), alpha=-1, patch_save_dir=None, patch_level=0):
    wsi = wsi_object.getOpenSlide()
    vis_level = wsi.get_best_level_for_downsample(downscale)
    file = h5py.File(hdf5_file_path, 'r')
    dset = file['coords']
    coords = dset[:] # type: ignore
    ...
    print('number of patches: {}'.format(len(coords))) # type: ignore
    ...
    heatmap = np.array(heatmap)
    heatmap = DrawMapFromCoords(heatmap, wsi_object, coords, patch_size, vis_level, indices=None,
                                 draw_grid=draw_grid, patch_save_dir=patch_save_dir, patch_level=patch_level) # type: ignore
    
    file.close()
    return heatmap

def SamplePatches(coords_file_path, save_file_path, wsi_object, 
    patch_level=0, custom_downsample=1, patch_size=256, sample_num=100, seed=1, stitch=True, verbose=0, mode='w'):
    ...
    if verbose>0:
        print('in .h5 file: total number of patches: {}'.format(len(coords))) # type: ignore
    ...
    np.random.seed(seed)
    indices = np.random.choice(np.arange(len(coords)), min(len(coords), sample_num), replace=False) # type: ignore
