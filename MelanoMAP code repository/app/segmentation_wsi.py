...
from ai_model.metastasis_prediction import ai_predict
...
def process_slide(wsi_name, results_df):
    ...
    try:
        print("[Step 1] Segmenting WSI and extracting patches...")
        seg_and_patch(wsi_name=wsi_name, staining=staining, save_dir=f'/data/preliminary_results/patches_{staining}/')

        print("[Step 2] Running handcrafted feature detection...")
        hc_prediction, hc_p_value = compute_hc_results(staining, os.path.join(image_dir, wsi_name.split('.')[0]))

        print("[Step 3] Running AI prediction for metastasis...")
        ai_prediction, ai_p_value = ai_predict(wsi_name)

        total_compute_time = round(time.time() - start_time, 2)

        return update_dataframe(results_df, wsi_name, staining, hc_prediction, hc_p_value, ai_prediction, ai_p_value, total_compute_time)
